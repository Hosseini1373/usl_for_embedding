...
def usl_t_pretrain_with_early_stopping(embeddings, device, patience):
...
def usl_t_selective_labels(model,embeddings,device):
...
def train(embeddings):
    ...
    model=usl_t_pretrain_with_early_stopping(embeddings,device,patience_cluster)
    selected_indices = usl_t_selective_labels(model,embeddings,device)
    return selected_indices
